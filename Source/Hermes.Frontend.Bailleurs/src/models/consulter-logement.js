export class ConsulterLogement {
    ...
    activate() {
      this._service.getLogementAConsulter().then(res => this.resultats = res);
    }
  
    search() {
      this._service.getLogementAConsulter().then(res => {
    ...
    navigateToDetail(pIndex) {
      ...
      this._router.navigateToRoute('logement', { id: id });
    }
